...
# Global variable to load data only once
global_data_list = None

def load_data():
    global global_data_list
    if global_data_list is not None:
...
def objective(trial):
    # Suggest hyperparameters (excluding architecture to reuse pretrained weights)
    lr = trial.suggest_float('lr', 1e-4, 5e-3, log=True)
    noise_std = trial.suggest_float('noise_std', 0.0, 0.05)
    edge_drop_rate = trial.suggest_float('edge_drop_rate', 0.0, 0.3)
    ...
    for fold in range(k_folds):
        ...
        if os.path.exists(pretrained_path):
            ...
            model.load_state_dict(state_dict, strict=False) # strict=False because heads might have changed or recon_head is present
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            if len(all_true) > 0:
                ...
                avg_val_r2 = calculate_r2(torch.cat(all_true), torch.cat(all_pred)).item()
    ...
    # In trial log
    print(f"[Trial {trial.number}] Mean R2: {mean_r2:.4f} | Params: {trial.params}")
...
if __name__ == "__main__":
    print("="*80)
    print("STARTING HPO: Pre-trained HGT + Transductive Shift (15 Trials)")
    print("="*80)
    
    study = optuna.create_study(direction='maximize', study_name="HGT_Transductive_Logistics")
    study.optimize(objective, n_trials=15)
    
    print("="*80)
    print("HPO COMPLETE.")
    print("Best Trial:")
    print("  Value (Mean R2):", study.best_trial.value)
    print("  Params: ")
