...
def analyze_nlp_keywords(task_name):
    ...
    if any(k in name_lower for k in ['purchasing', 'procurement']):
        return 'material'
    elif any(k in name_lower for k in ['transportation', 'delivery', 'shipping', 'mobilisation']):
        return 'transportation'
    elif any(k in name_lower for k in ['seabed', 'dredging']):
        return 'handling'
    elif any(k in name_lower for k in ['fabrication', 'welding', 'assembly', 'concrete', 'formworks', 'pile', 'paving', 'barriers', 'culverts']):
        return 'material'
    elif any(k in name_lower for k in ['fixed yard', 'containers', 'fence', 'pegging', 'network service', 'platform']):
        return 'facility'
    elif any(k in name_lower for k in ['testing', 'survey', 'inspection', 'commissioning']):
...
def main():
    ...
    file_path = os.path.join(script_dir, 'raw', 'DSLIB', 'Excel', 'C2012-08 Sea Electricity.xlsx')
    out_dir = os.path.join(script_dir, 'processed', 'C2012-08')
    ...
    for i in range(1, len(df_schedule)):
        ...
        if res_demand_str and str(res_demand_str) != 'nan':
            ...
            for rp in res_parts:
                ...
                if match:
                    ...
                    labor_keywords = ['laborer', 'labourer', 'worker', 'coordinator', 'skipper', 'welder', 'engineer', 'team', 'diver', 'diving']
                    if any(k in rname_lower for k in labor_keywords):
        ...
        if classification == 'material':
            costs['material'] += fixed_cost + var_cost
        elif classification == 'transportation':
            costs['transportation'] += fixed_cost * 0.8 + var_cost * 0.5
            costs['inventory_holding'] += fixed_cost * 0.2 + var_cost * 0.5
        elif classification == 'handling':
            costs['packaging'] += fixed_cost * 0.7 + var_cost
            costs['energy'] += fixed_cost * 0.3
        elif classification == 'facility':
            costs['facility'] += fixed_cost * 0.8 + var_cost * 0.5
            costs['utilities'] += fixed_cost * 0.2 + var_cost * 0.5
        elif classification == 'regulatory_compliance':
            costs['regulatory_compliance'] += fixed_cost
        elif classification == 'testing_inspection':
            costs['testing_inspection'] += fixed_cost + var_cost
        else:
        ...
        weather_risk = variance * 0.85
        complexity = variance * 0.15
        ...
        for p in preds:
            edges_rows.append({
                "source_id": f"C2012-08_{p['target_id']}",
                "target_id": f"C2012-08_{task_id}",
        ...
        schedules_rows.append({
            ...
            "predecessors": [f"C2012-08_{p['target_id']}" for p in preds],
            "successors": []
        })
    ...
    print(f"Successfully processed {len(output_tasks)} tasks for C2012-08.")
