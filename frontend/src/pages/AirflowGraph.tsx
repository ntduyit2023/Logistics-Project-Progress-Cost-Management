...
import { X, Clock, DollarSign, Activity, AlertTriangle } from 'lucide-react';
...
const AirflowGraph = ({ projectId, tasks, dependencies, selectedOptionModes, criticalityIndices }: any) => {
  ...
  const get72Features = (task: any, mode: number, isCritical: boolean) => {
    ...
    const riskVal = criticalityIndices[task.id] || (isCritical ? 0.90 : 0.15);
  ...
  const { initialNodesLayout, initialEdgesLayout } = useMemo(() => {
    ...
    // Slicing 35 tasks for beautiful and responsive UI rendering
    const displayTasks = tasks.slice(0, 35);
    const displayTaskIds = new Set(displayTasks.map(t => t.id));
    ...
    const rawNodes = displayTasks.map((task: any, idx: number) => {
      ...
      const wbs = task.id.split('_')[1] || task.id;
      const isCritical = criticalityIndices && criticalityIndices[task.id] > 0.75;
      ...
      return {
        ...
        data: {
          task_id: task.id,
          wbs,
          task_name: task.name || getTaskNameByWbs(wbs),
          duration,
          total_cost: cost,
    ...
    const rawEdges = dependencies
      .filter((dep: any) => {
        const predId = String(dep[0]);
        const succId = String(dep[1]);
        return displayTaskIds.has(predId) && displayTaskIds.has(succId);
      })
      .map((dep: any) => ({
        id: `e-${dep[0]}-${dep[1]}`,
        source: String(dep[0]),
