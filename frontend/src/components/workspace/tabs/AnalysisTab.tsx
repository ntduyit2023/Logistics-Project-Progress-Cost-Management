...
import { ResponsiveContainer, ComposedChart, CartesianGrid, XAxis, YAxis, Tooltip, Legend, Bar, Area, Line, AreaChart } from 'recharts';
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fadeIn">
      ...
      <div className="lg:col-span-3 bg-white p-5 rounded-xl border border-slate-200 shadow-sm h-[300px] flex flex-col">
        <div className="mb-4 shrink-0">
          <h3 className="font-bold text-slate-800">Biểu đồ Phân bổ Công việc (Task Allocation)</h3>
          <p className="text-xs text-slate-500">Mật độ công việc chạy song song theo từng ngày dự án</p>
        </div>
        <div className="flex-1 min-h-0">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={combinedData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Tooltip
                contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}
                formatter={(val: any) => [`${val} tasks`, 'Đang thực hiện']}
                labelFormatter={(val) => `Ngày: ${val.split('-')[2]}/${val.split('-')[1]}`}
              />
