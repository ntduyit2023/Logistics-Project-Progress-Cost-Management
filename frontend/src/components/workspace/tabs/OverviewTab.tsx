  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Total Tasks" value={`${tasks?.length || 0} công việc`} icon={Layers} color="bg-blue-500" />
        <StatCard title="Dependencies" value={`${dependencies?.length || 0} liên kết`} icon={GitCommit} color="bg-emerald-500" />
        <StatCard title="Estimated Makespan" value={`${(displayMakespan || 0).toFixed(0)}h`} icon={Clock} color="bg-amber-500" />
        <StatCard title="Total Project Cost" value={`$${((displayCost || 0) / 1000).toFixed(1)}k`} icon={Activity} color="bg-purple-500" />
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm mt-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-sm">
          <div>
            <p className="text-slate-500 font-semibold mb-1">Project Name</p>
            <p className="font-bold text-slate-800">{projectData?.project_name || "N/A"}</p>
          </div>
          <div>
            <p className="text-slate-500 font-semibold mb-1">Project ID</p>
